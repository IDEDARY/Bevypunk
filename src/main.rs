...
fn setup(mut commands: Commands, assets: Res<AssetCache>) {
    ...
    commands.spawn((
        ...
        ui.spawn((
            UiLink::path("Root/Secondary"),
            UiLayout::Window::new().pos(Prc((50.0, 0.0))).size(Prc((50.0, 50.0))).pack(),
            UiTreeBundle::<NoData, NoData, MenuUi>::from(UiTree::new("Bevypunk2")),
            //MovableByCamera,
        )).with_children(|ui| {
            ...
            ui.spawn((
                MenuUi,
                UiLink::path("Root/Secondary"),
                UiLayout::Window::new().pos(Prc((50.0, 0.0))).size(Prc((50.0, 100.0))).pack(),
            ));
